...
/* Sphere 클래스를 상속 받는 Jupiter 클래스 생성자 정의 */
Jupiter::Jupiter() :Sphere(23, 45, 45) //구의 중심 좌표 설정
{
	init();
}

/* Sphere 클래스를 상속 받는 Jupiter 클래스 소멸자 정의 */
Jupiter::~Jupiter()
{
	Sphere::~Sphere();
}
...
void Jupiter::init()
{
	...
	unsigned char * data = loadBMPRaw("jupiter.bmp", width, height, false);	//"jupiter.bmp"파일을 불러온다
