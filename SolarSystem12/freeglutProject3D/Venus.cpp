...
/* Sphere 클래스를 상속 받는 Venus 클래스 생성자 정의 */
Venus::Venus() : Sphere(3.5, 45, 45) //구의 중심 좌표 설정
{
	init();
}

/* Sphere 클래스를 상속 받는 Venus 클래스 소멸자 정의 */
Venus::~Venus()
{
	Sphere::~Sphere();
}
...
void Venus::init()
{
	...
	unsigned char * data = loadBMPRaw("venus.bmp", width, height, false);	//"venus.bmp" 파일을 불러온다
