...
/* Sphere 클래스를 상속 받는 Naptune 클래스 생성자 정의 */
Naptune::Naptune() : Sphere(9, 45, 45) //구의 중심 좌표 설정
{
	init();
}

/* Sphere 클래스를 상속 받는 Naptune 클래스 소멸자 정의 */
Naptune::~Naptune()
{
	Sphere::~Sphere();
}
...
void Naptune::init()
{
	...
	unsigned char * data = loadBMPRaw("neptune.bmp", width, height, false);	//"neptune.bmp"파일을 불러온다
